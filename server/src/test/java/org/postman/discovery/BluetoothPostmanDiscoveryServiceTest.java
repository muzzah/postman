...
public class BluetoothPostmanDiscoveryServiceTest {
    ...
    public void shouldStartBroadcast() throws UnknownHostException {
        TestSubscriber testSubscriber = service.startServiceBroadcast("servicetype", "name", 22, InetAddress.getLocalHost()).test();
    ...
    public void shouldNotifyOfErrorWhenDiscovering() throws UnknownHostException {
        ...
        TestSubscriber testSubscriber = service.discoverService("servicetype", "name", InetAddress.getLocalHost()).test();
        scheduler.triggerActions();
        testSubscriber.assertError(RuntimeException.class);
