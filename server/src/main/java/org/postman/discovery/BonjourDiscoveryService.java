...
public class BonjourDiscoveryService implements PostmanDiscoveryService {
    ...
    public BonjourDiscoveryService(Scheduler io) {
        this.io = io;
    }

    @Override
    public Flowable<PostmanBroadcastEvent> startServiceBroadcast(@NonNull String serviceType, @NonNull String serviceName, int port, @NonNull InetAddress hostAddress) {
        return Flowable.<PostmanBroadcastEvent>fromPublisher(
                subscriber -> {
                    try {
                        ...
                        ServiceInfo serviceInfo = ServiceInfo.create(serviceType, serviceName, port, 100, 100, true,
                                "");
    ...
    @Override
    public Flowable<PostmanDiscoveryEvent> discoverService(@NonNull String serviceType, @NonNull String serviceName, @NonNull InetAddress addressToSearchOn) {
        return Flowable.<PostmanDiscoveryEvent>fromPublisher(
                subscriber -> {
                    try {
                        ...
                        jmdns.addServiceListener(serviceType, siiaServiceListener);
                        ...
                        if (nonNull(jmdns)) {
                            jmdns.removeServiceListener(serviceType, siiaServiceListener);
                            closeJmdnsQuietly();
                        }
                    }
                }
